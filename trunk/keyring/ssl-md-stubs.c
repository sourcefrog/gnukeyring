...
static void
GLib_CrMD_clean(unsigned short cmd, void *PBP, unsigned short flags)
{
    if (reg_a4 && libref) {
	GLibClose(libref);
	libref = 0;
...
static void *hook
  __attribute__ ((section ("ehook"), unused)) = GLib_CrMD_clean;

#else

static void __attribute__ ((destructor, unused))
GLib_CrMD_clean(void)
{
    if (libref) {
	GLibClose(libref);
	libref = 0;
